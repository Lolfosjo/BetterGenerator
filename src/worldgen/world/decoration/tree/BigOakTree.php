...
class BigOakTree implements TreeType {
    ...
    public function __construct() {
        $this->log_type = VanillaBlocks::OAK_LOG();
        $this->leaves_type = VanillaBlocks::OAK_LEAVES();
    ...
    private function canPlaceOn(Block $block): bool {
        $id = $block->getTypeId();
        return $id === BlockTypeIds::GRASS || 
               $id === BlockTypeIds::DIRT || 
               $id === BlockTypeIds::PODZOL ||
               $id === BlockTypeIds::COARSE_DIRT ||
               $id === BlockTypeIds::MYCELIUM;
    }

    private function canPlace(int $base_x, int $base_y, int $base_z, ChunkManager $world): bool {
    ...
    public function generate(ChunkManager $world, Random $random, int $source_x, int $source_y, int $source_z): bool {
        ...
        if (!$this->canPlaceOn($world->getBlockAt($source_x, $source_y - 1, $source_z)) || 
            !$this->canPlace($source_x, $source_y, $source_z, $world)) {
            return false;
        }
                ...
                for ($x = -$node_distance; $x <= $node_distance; ++$x) {
                    for ($z = -$node_distance; $z <= $node_distance; ++$z) {
                        ...
                        if ($size_x * $size_x + $size_z * $size_z <= $size * $size) {
                            $block = $world->getBlockAt($node->x + $x, $node->y + $y, $node->z + $z);
                            if (array_key_exists($block->getTypeId(), $this->overridables)) {
                                $world->setBlockAt($node->x + $x, $node->y + $y, $node->z + $z, $this->leaves_type);
                            }
                        }
                    }
                }
        ...
        for ($y = 0; $y < $this->trunk_height; ++$y) {
            $world->setBlockAt($source_x, $source_y + $y, $source_z, $this->log_type);
        }
        ...
        foreach ($leaf_nodes as $node) {
            if ((float)($node->branch_y - $source_y) >= $this->height * 0.2) {
                ...
                $max_distance = max(
                    abs($branch->getFloorY()), 
                    max(abs($branch->getFloorX()), abs($branch->getFloorZ()))
                );
                ...
                if ($max_distance > 0) {
                    ...
                    for ($i = 0; $i <= $max_distance; ++$i) {
                        $branch_pos = $base->add(0.5 + $i * $dx, 0.5 + $i * $dy, 0.5 + $i * $dz);
                        $x = abs($branch_pos->getFloorX() - $base->getFloorX());
                        $z = abs($branch_pos->getFloorZ() - $base->getFloorZ());
                        $max = max($x, $z);
                        
                        $direction = $max > 0 ? ($max === $x ? Axis::X : Axis::Z) : Axis::Y;
                        
                        $log = clone $this->log_type;
                        if ($log instanceof Wood) {
                            $log->setAxis($direction);
                        }
                        
                        $world->setBlockAt(
                            $branch_pos->getFloorX(), 
                            $branch_pos->getFloorY(), 
                            $branch_pos->getFloorZ(), 
                            $log
                        );
                    }
                }
    ...
    private function countAvailableBlocks(Vector3 $from, Vector3 $to, ChunkManager $world): int {
        ...
        $max_distance = max(
            abs($target->getFloorY()), 
            max(abs($target->getFloorX()), abs($target->getFloorZ()))
        );
        ...
        if ($max_distance > 0) {
            ...
            for ($i = 0; $i <= $max_distance; ++$i, ++$n) {
                $target_pos = $from->add(0.5 + $i * $dx, 0.5 + $i * $dy, 0.5 + $i * $dz);
                $target_floorY = $target_pos->getFloorY();
                
                if ($target_floorY < 0 || $target_floorY > $height) {
                    return $n;
                }
                
                $block = $world->getBlockAt(
                    $target_pos->getFloorX(), 
                    $target_pos->getFloorY(), 
                    $target_pos->getFloorZ()
                );
        ...
        for ($l = --$y - $block_y; $l >= 0; --$l, --$y) {
            ...
            if ($f >= 0.0) {
                for ($i = 0; $i < $node_count; ++$i) {
                    ...
                    if ($this->countAvailableBlocks(
                        new Vector3($x, $y, $z), 
                        new Vector3($x, $y + $this->max_leaf_distance, $z), 
                        $world
                    ) === -1) {
                        $off_x = $block_x - $x;
                        $off_z = $block_z - $z;
...
/**
 * Repräsentiert einen Laubknoten mit Astbasis
 */
