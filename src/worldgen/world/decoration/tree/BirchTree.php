...
class BirchTree implements TreeType
{
	 ...
	 * @param int|null $min_height minimum height (optional)
	 * @param int|null $max_height maximum height (optional)
	 */
	public function __construct(Random $random, ?int $min_height = null, ?int $max_height = null)
	{
		...
		// If min/max heights are provided, use them; otherwise use default 4-7
		$min = $min_height ?? 4;
		$max = $max_height ?? 7;
		$range = $max - $min;
		
		$this->setHeight($random->nextBoundedInt($range) + $min);
	...
	/**
	 * Attempts to grow this tree at its current location. If successful, blocks are set to wood and leaves.
	 *
	...
	{
		if ($this->cannotGenerateAt($x, $y, $z, $world)) {
			return false;
		}

		// generate the leaves
		for ($leaf_y = $y + $this->height - 3; $leaf_y <= $y + $this->height; ++$leaf_y) {
			$n = $leaf_y - ($y + $this->height);
			$radius = (int) (1 - $n / 2);
			for ($leaf_x = $x - $radius; $leaf_x <= $x + $radius; ++$leaf_x) {
				for ($leaf_z = $z - $radius; $leaf_z <= $z + $radius; ++$leaf_z) {
					if (
						abs($leaf_x - $x) !== $radius
						|| abs($leaf_z - $z) !== $radius
						|| ($random->nextBoolean() && $n !== 0)
					) {
						$this->replaceIfAirOrLeaves($leaf_x, $leaf_y, $leaf_z, $this->leaves_type, $world);
					}
				}
			}
		}
		...
		// generate the trunk
		for ($trunk_y = 0; $trunk_y < $this->height; ++$trunk_y) {
			$this->replaceIfAirOrLeaves($x, $y + $trunk_y, $z, $this->log_type, $world);
		}
	...
	protected function replaceIfAirOrLeaves(int $x, int $y, int $z, Block $new_material, ChunkManager $world): void
	{
		...
		if ($old_material->getTypeId() === BlockTypeIds::AIR || $old_material instanceof Leaves) {
			$world->setBlockAt($x, $y, $z, $new_material);
		}
	}
}
